...
def find_points(img_label):
    ...
    print("Coordinates for Right mouth corner are {}".format(mouth_right))
    
    # Left mouth corner
    mouth_left = np.flip(np.unravel_index(np.argmax(mouth * grad_left), img_label.shape))
    print("Coordinates for Left mouth corner are {}".format(mouth_left))
    ...
    print("Coordinates for Right eye corner are {}".format(eye_right))
    
    # Left eye left corner
    eye_left = np.flip(np.unravel_index(np.argmax(eyes * grad_left), img_label.shape))
    print("Coordinates for Left eye corner are {}".format(eye_left))
    ...
    r1 = max(mouth_left[0] - eye_left[0],0) ** 2
    r2 = max(eye_right[0] - mouth_right[0],0) ** 2
    
    x_nose_tip = int(nose_left + r1 / (r1 + r2) * (nose_right - nose_left))
    y_nose_tip = int(nose_up + 0.8 * (nose_down - nose_up))
    
    nose_tip = (x_nose_tip, y_nose_tip)
    print("Coordinates for nose tip are {}".format(nose_tip))
    ...
    chin = (x_chin, y_chin)
    print("Coordinates for chin are {}".format(chin))
